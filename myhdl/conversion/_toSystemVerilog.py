...
""" myhdl toSystemVerilog conversion module.
...
from myhdl._extractHierarchy import (_HierExtr, _isMem, _getMemInfo,
                                     _UserSystemVerilogCode, _userCodeMap)
...
def _checkArgs(arglist):
    for arg in arglist:
        if not isinstance(arg, (GeneratorType, _Instantiator, _UserSystemVerilogCode)):
            raise ToSystemVerilogError(_error.ArgType, arg)
...
class _ToSystemVerilogConvertor(object):
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        if _traceSignals._tracing:
            raise ToSystemVerilogError("Cannot use toSystemVerilog while tracing signals")
        if not isinstance(func, _Block):
            if not callable(func):
                raise ToSystemVerilogError(_error.FirstArgType, "got %s" % type(func))
        ...
        if isinstance(func, _Block):
            ...
            warnings.warn(
                "\n    toSystemVerilog(): Deprecated usage: See http://dev.myhdl.org/meps/mep-114.html", stacklevel=2)
            try:
        ...
        if len(intf.argnames) > 0 and not toSystemVerilog.no_testbench:
...
toSystemVerilog = _ToSystemVerilogConvertor()
...
def _writeFileHeader(f, fn, ts):
    ...
    if not toSystemVerilog.no_myhdl_header:
        print(string.Template(myhdl_header).substitute(vars), file=f)
    if toSystemVerilog.header:
        print(string.Template(toSystemVerilog.header).substitute(vars), file=f)
    print(file=f)
            ...
            if s._read:
                if not isinstance(s, _TristateSignal):
                    warnings.warn("%s: %s" % (_error.OutputPortRead, portname),
                                  category=ToSystemVerilogWarning
                                  )
            if isinstance(s, _TristateSignal):
                print("    inout logic %s%s%s;" % (p, r, portname), file=f)
            else:
                print("    output logic %s%s%s;" % (p, r, portname), file=f)
            if s._driven == 'reg':
                print("    output logic %s%s%s;" % (p, r, portname), file=f)
            else:
                print("    input logic %s%s%s;" % (p, r, portname), file=f)
        else:
            if not s._read:
...
def _writeSigDecls(f, intf, siglist, memlist):
    ...
    for s in siglist:
        ...
        if s._driven:
            ...
            #if not toSystemVerilog.initial_values or k == 'wire':
            if not toSystemVerilog.initial_values:
                print("%s %s%s%s;" % (k, p, r, s._name), file=f)
            else:
                if isinstance(s._init, myhdl._enum.EnumItemType):
                    print("%s %s%s%s = %s;" %
                          (k, p, r, s._name, s._init._toSystemVerilog()), file=f)
                else:
                    print("%s %s%s%s = %s;" %
                          (k, p, r, s._name, _intRepr(s._init)), file=f)
        elif s._read:
            # the original exception
            # raise ToSystemVerilogError(_error.UndrivenSignal, s._name)
            ...
            print("logic %s%s;" % (r, s._name), file=f)
    # print(file=f)
    for m in memlist:
        if not m._used:
            continue
        ...
        if m._driven:
            ...
            #if toSystemVerilog.initial_values and not k == 'wire':
            if toSystemVerilog.initial_values:
    ...
    for s in constwires:
        if s._type in (bool, intbv):
            ...
            raise ToSystemVerilogError("Unexpected type for constant signal", s._name)
        c_len = s._nrbits
        c_str = "%s" % c
    ...
    for s in siglist:
        if hasattr(s, 'toSystemVerilog') and s._driven:
            print(s.toSystemVerilog(), file=f)
    print(file=f)
...
def _writeTestBench(f, intf, trace=False):
    ...
    for portname in intf.argnames:
        ...
        if s._driven:
            ...
            print("logic %s%s;" % (r, portname), file=f)
...
def _convertGens(genlist, vfile):
    ...
    funcBuf = StringIO()
    for tree in genlist:
        if isinstance(tree, _UserSystemVerilogCode):
...
class _ConvertVisitor(ast.NodeVisitor, _ConversionMixin):
    ...
    def raiseError(self, node, kind, msg=""):
        ...
        raise ToSystemVerilogError(kind, msg, info)
    ...
    def writeSensitivityList(self, senslist):
        sep = ', '
        if toSystemVerilog.standard == '1995':
            sep = ' or '
        self.write("@(")
        for e in senslist[:-1]:
    ...
    def visit_BinOp(self, node):
        if isinstance(node.op, ast.Mod) and self.context == _context.PRINT:
            ...
            if isinstance(node.op, ast.RShift):
                ...
                # is actually required by standard SystemVerilog.
...
#        self.writeline()
        ...
        for test, suite in node.tests:
            ...
            if isinstance(item, EnumItemType):
                self.write(item._toSystemVerilog(dontcare=True))
            else:
...
class _ConvertAlwaysCombVisitor(_ConvertVisitor):
    ...
    def __init__(self, tree, blockBuf, funcBuf):
        _ConvertVisitor.__init__(self, tree, blockBuf)
        if toSystemVerilog.prefer_blocking_assignments:
            self.okSigAss = False
        self.funcBuf = funcBuf
